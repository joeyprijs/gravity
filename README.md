...
Within the unheaded list the order stays **authored**, and the convention is compass order, clockwise from north — for a house's doors as much as a junction's roads, since it is what makes the arrows read as a sweep rather than a scatter. Sort by bearing with the wrap at *due north*, so a way bearing slightly west of north leads instead of trailing after south; two that lead the same way sit together. No `isBack` on any of them: it sinks an option to the bottom, which is right where "back" is the odd one out (a dead-end cottage, the way out of a building) and wrong on a road or a door, where the way you came is a direction like any other. Thresholds are exempt — they carry no arrow, so Entrances and Exits stay as authored.
