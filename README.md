...
Within the unheaded list, roads stay in **authored** order, and the convention for a junction is compass order — N, E, S, W, clockwise from north — with no `isBack` on any of them. `isBack` sinks an option to the bottom, which is right where "back" is the odd one out (a dead-end cottage, a room off a corridor) and wrong at a crossroads, where the way you came is just one direction among several. The engine deliberately does *not* sort by map bearing: `mapDefinitions` are authored to make the map read well, and tying option order to them would mean nudging a box to fix an overlap silently reorders the interactions panel.
