...
**Thresholds get no arrow.** Crossing one is "in" or "out", never "east", so nothing under **Entrances** or **Exits** is marked. That is the same line the sections themselves are drawn on: the arrow is for movement *within* a continuous space, and the headings are for movement between two of them. Indoors the compass reads less literally than on a road — nobody thinks of their kitchen as west — but a house of eight rooms has a shape, and this is what carries it. Two rooms down the same side of a house do point the same way, which is honest rather than useful; the label is what tells them apart.

Within the unheaded list the order stays **authored**, and the convention is compass order, clockwise from north — for a house's doors as much as a junction's roads, since it is what makes the arrows read as a sweep rather than a scatter. Sort by bearing with the wrap at *due north*, so a way bearing slightly west of north leads instead of trailing after south; two that lead the same way sit together. No `isBack` among them: it sinks an option to the bottom, which pulls one road out of the sequence the rest read in. It stays right wherever "back" genuinely is the odd one out — a dead-end cottage, a room whose only exit is the way you came, the way out of a building — and it is more than a sort key there: the curator identifies a museum room's way out by finding its single `isBack` option, so those rooms need it. The rule is only "not where it would compete with compass order". Thresholds are exempt either way — they carry no arrow, so Entrances and Exits stay as authored.
