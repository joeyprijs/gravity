...
**What the minimap draws.** Inside a building: that building's visited rooms, plus the outdoor ground its doors open onto — in a one-room shop, the shop and the street outside. Outdoors: a **viewport** onto the open world, centered on the player, spanning `minimapRadius` world units in every direction (omit the field and the outdoor map falls back to fitting everything known into the square). Walking scrolls it; the world getting bigger never shrinks what you can read.

**Doorways.** A short light bar on the wall two boxes share marks where a `navigate` crosses it — from a street into a cottage, or between two rooms of one house. Derived, never authored: the door sits at the midpoint of the shared edge, so rooms and buildings are laid out touching, as a floor plan is. Only passages into or inside a building are marked; two roads meeting is an open edge. A house that backs onto the square but opens onto the lane shows its door on the lane, which is the misreading the mark exists to prevent. Both boxes must already be drawn, so a door into an unwalked room is never shown before the room is.
...
**What the interactions panel does with it.** Nothing: a building's threshold is a move like any other. From the road a building is a square on the minimap, and from inside the ground outside its door is drawn too, so going in or stepping out sits among the roads and room doors in authored order, with its own arrow (a teleport out is a return, and sinks to the foot of the list like any back option). Sections follow from what an option's pipeline does, never from where it leads or how its label is worded.

**Naming a box.** At minimap scale a name is unreadable, so places are drawn as unlabeled boxes and their labels come back as **hover titles** — the same trick the top bar uses, where a stat's icon keeps its label as a hover name. Point at a box to learn what it is; it is the only thing distinguishing one from another at that size, so every box carries one.

**A move shows which way it goes.** Any option in the unheaded list — a road between two outdoor places, or a door between two rooms of one building — carries a compass arrow on its trailing edge, derived from the two scenes' `mapDefinitions`, read **edge to edge**: a box that overlaps yours side to side is above or below you, one that overlaps top to bottom is left or right, and only a box off in a corner is placed by the wider of its two gaps. The eye reads the map the same way — an inn hugging the top edge of a wide square is *up*, though the line between the two centres runs past the diagonal. Rounded to the nearest **cardinal**: one glyph turned a quarter-turn per point, so a direction never needs its own art. Four points, not eight: the eye reads "up" and "right" instantly and has to stop and work out "up-and-right", so a road running north-east is shown as north. Roads are still *ordered* on eight points, which needs the finer resolution to be deterministic. Because the map is where the world records direction, the prose doesn't: labels read "Take the forge lane", not "Take the forge lane east", which keeps the one piece of navigational meaning in the game out of a string a translator has to get right (the point's name rides along for screen readers).

**Doors get arrows too.** The arrow is for movement across the space the minimap draws, and a building's door crosses it either way: the building is a square on the outdoor map, the ground outside its door is on the indoor one. So "go into the smithy" points where the smithy stands, the same as "take the forge lane" points along the lane. Indoors the compass reads less literally than on a road — nobody thinks of their kitchen as west — but a house of eight rooms has a shape, and this is what carries it. Two rooms down the same side of a house do point the same way, which is honest rather than useful; the label is what tells them apart.

Within the unheaded list the order stays **authored**, and the convention is compass order, clockwise from north — for a house's doors as much as a junction's roads, since it is what makes the arrows read as a sweep rather than a scatter. Sort by bearing with the wrap at *due north*, so a way bearing slightly west of north leads instead of trailing after south; two that lead the same way sit together. No `isBack` among them: it sinks an option to the bottom, which pulls one road out of the sequence the rest read in. It stays right wherever "back" genuinely is the odd one out — a dead-end cottage, a room whose only exit is the way you came — and it is more than a sort key there: the curator identifies a museum room's way out by finding its single `isBack` option, so those rooms need it. The rule is only "not where it would compete with compass order". Doors into buildings are in the sweep like any road, so a lane's cottages are authored round the compass with the roads leaving it.

The engine deliberately does *not* sort by map bearing itself: `mapDefinitions` are authored to make the map read well, and tying option order to them would mean nudging a box to fix an overlap silently reorders the interactions panel. It draws the arrow from geometry and leaves the sequence to the author.
