...
Within the unheaded list, roads stay in **authored** order, and the convention outdoors is compass order — N, NE, E, SE, S, SW, W, NW, clockwise from north — with no `isBack` on any of them. Direction means the compass point, not the exact angle: a road bearing slightly west of due north is north, and leads. `isBack` sinks an option to the bottom, which is right where "back" is the odd one out (a dead-end cottage, a room off a corridor, the way out of a building) and wrong on a road, where the way you came is a direction like any other. The engine deliberately does *not* sort by map bearing: `mapDefinitions` are authored to make the map read well, and tying option order to them would mean nudging a box to fix an overlap silently reorders the interactions panel.
