...
- **Outdoor roads are authored in compass order** — N, NE, E, SE, S, SW, W, NW, clockwise from north — and none of them is flagged `isBack`. Read the direction as a *compass point*, not a precise angle: a road bearing a little west of due north is still north and comes first, not last. `isBack` is for where "back" is genuinely the odd one out (a dead-end cottage, a room off a corridor, the way out of a building); on a road it sinks the way you came out of the order the others read in, which is exactly what makes a list feel arbitrary. It applies to a two-road stretch as much as a crossroads — a road you came in by is still a direction. Order is left to the author on purpose: the engine could sort by `mapDefinitions` bearing, but that would tie panel order to coordinates authored for *drawing*, so nudging a box to fix a map overlap would silently reorder the options. Doors (**Entrances**) are not directions and stay in authored order.
