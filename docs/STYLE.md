...
- Registry constants live in `core/config.js` and are the only way to spell their strings in JS: CSS classes (`CSS`), element ids (`EL`), flag keys (`FLAG_KEYS`), check-state keys (`CHECK_KEYS`), log labels (`LOG`), mission statuses (`MISSION_STATUS`), item types (`ITEM_TYPES`). An inline `'scene-options'` or `'skill_dc_' + x` in a module is a defect: a typo would silently mint a new key.
...
- **Order is the author's, on purpose.** The engine could sort by `mapDefinitions` bearing itself, but that would tie panel order to coordinates authored for *drawing* — nudging a box to fix a map overlap would silently reorder the options. It draws the arrow from geometry and leaves the sequence alone. The geometry is read edge to edge: a box drawn along your top edge is north however far along that edge it sits, so place a building's box against the side of the road it is entered from and the arrow follows.
