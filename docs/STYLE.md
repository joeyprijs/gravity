...
- **`isBack` is for where "back" is genuinely the odd one out** — a dead-end cottage, the way out of a building. Never on a road or a door between rooms: it sinks that option out of the compass order the others read in, which is exactly what makes a list feel arbitrary.
