...
- **Outdoor roads are authored in compass order** — N, NE, E, SE, S, SW, W, NW, clockwise from north — and none of them is flagged `isBack`. Read the direction as a *compass point*, not a precise angle: a road bearing a little west of due north is still north and comes first, not last.
- **A road's prose does not name its direction** — write "Take the forge lane", not "Take the forge lane east". The engine derives the direction from `mapDefinitions` and draws it as an arrow on any option that stays on one side of a threshold (a road, or a door between two rooms of one building), so a label that says it too is saying it twice and can drift out of step with the map. Direction words are fine inside a *name* ("the East Row", "the north road") — that is what the road is called, not a claim about which way you are facing. `isBack` is for where "back" is genuinely the odd one out (a dead-end cottage, a room off a corridor, the way out of a building); on a road it sinks the way you came out of the order the others read in, which is exactly what makes a list feel arbitrary. It applies to a two-road stretch as much as a crossroads — a road you came in by is still a direction. Order is left to the author on purpose: the engine could sort by `mapDefinitions` bearing, but that would tie panel order to coordinates authored for *drawing*, so nudging a box to fix a map overlap would silently reorder the options. Doors (**Entrances**) are not directions and stay in authored order.
