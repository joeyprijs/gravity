...
`index.html` loads a single entry point, `src/core/engine.js`. In normal play `DOMContentLoaded` runs `bootGame()` (for the Studio live-preview path, see below):

0. **Game selection** — an optional `games.json` at the web root registers multiple game manifests (`{ id, name, description, manifest }`). `?game=<id>` picks one; several games without a valid pick render the chooser overlay (`src/screens/game-select.js`, whose text comes from `games.json` since no locale is loaded yet). A missing or single-entry registry boots straight into the only (or default `data/index.json`) manifest — single-game deployments never see any of this. Each game is a self-contained data pack (own manifest, rules, locale, scenes/npcs/items); the demo lives in `data/`, other games under `games/<id>/`. Studio currently edits only the demo pack.
1. **Construct subsystems** — combat, dialogue, quests, narrative log, scene renderer, UI manager. Each receives the engine instance.
2. **`init()`** registers the built-in actions, then loads the selected manifest, resolves the active language (see *Localisation*), and fetches every registered asset in parallel. NPC `carriedItems` are normalized at load to `{ item, amount }` objects (`amount: null` = unlimited), so data files may use the string shorthand but consumers only ever see one shape.
