...
- **Panel ↔ minimap link:** the minimap draws unlabeled boxes (a name is unreadable at that scale), so the interactions panel serves as its legend — pointing at an option that navigates lights that place up. The seam is one attribute: `systems/scene.js` puts the resolved `navigate` destination on the button as `data-destination` and knows nothing else; `MapManager.setup()` delegates `mousemove`/`focusin` from `#scene-options-panel` (delegated, because the panel rebuilds its buttons on every non-navigating option click) and `highlightPlace()` decides what a destination means for the map — the room if it is drawn, else the building holding it, and nothing at all if the map isn't drawing it. Minimap nodes carry `data-place` (scene id for a room, building key for a building square) plus their name as a hover `title`, so the lookup works from either side. Skill-check destinations are excluded here exactly as they are from reveal. **`mousemove`, not `mouseover`, and that is load-bearing:** taking an option rebuilds the panel under a cursor that has not moved, and the browser answers that with `mouseover` on whichever button lands beneath it — which would light a place the player never asked about the instant they arrived somewhere else. Pointing is a movement; arriving under a stationary pointer is not. Since it now fires per pixel, `highlightPlace` returns early when the answer is unchanged, and clears `_highlightedPlace` whenever the canvas is rebuilt (the old nodes held the class).
