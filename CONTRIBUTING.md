...
3.  **Cross-Subsystem Decoupling:** The stateful subsystems (Combat, Narrative, Scene, Quests, Dialogue) must remain completely decoupled — they never import one another and never reach into another's state; communication is routed through the Event Bus (`engine.emit` and `engine.on`) or thin coordinator delegates on `RPGEngine`. The pure, stateless helpers (`dice.js`, `condition.js`, `skill-checks.js`) are the exception: they have no engine/DOM/state access, so subsystems import them directly like any library function.
